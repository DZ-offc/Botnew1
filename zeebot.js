...
//━━━━━━━━━━━━━━━[ CONNECTION 1 ]━━━━━━━━━━━━━━━\\            
        ...
        //+++ || FAKE TROLI         
           const troli =  {key: { fromMe: false,remoteJid: "status@broadcast", participant: '0@s.whatsapp.net'}, message: {orderMessage: {itemCount: 2021, status: 200, thumbnail: tamnel, surface: 200, message: `➥ Made By ${Miminnya}\n➥ Hallo ${pushname}`, orderTitle: 'LordPebri', sellerJid: '0@s.whatsapp.net'} } }    
...
switch (command) {         
    ...
    case'donasi':
    reply(mess.wait) 
    dns = fs.readFileSync('./media/donasi.jpg')
    txt_nya =`*Donasi woy seikhlas nya aja🤙
    "Yang donasi semoga dapet pahala banyak amin"
    *Dana* = 081273900363 
    *Pulsa* = 081273900363 
    _thanks buat yang donasi_*`
    zee.sendMessage(from, dns, image, {quoted: mek, caption: txt_nya}) 
    break
    case 'bc': case 'broadcast':
         ...
         for (let _ of anu) {
       	 tes = `${body.slice(4)}`
         zee.sendMessage(_.jid, bc, { contentText: `${tes}`, footerText: `© DZ-offc `, buttons: [{buttonId: `${prefix}sc`,buttonText:{displayText: '</SCBOT>'},type:1},{buttonId: `${prefix}owner`,buttonText:{displayText:'OWNER💪🗿'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: tamnel, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu) {
             	textt = `${body.slice(4)}`
             zee.sendMessage(_.jid, { contentText: `${textt}`, footerText: `© DZ-offc `, buttons: [{buttonId: `${prefix}sc`,buttonText:{displayText: '</SCBOT>'},type:1},{buttonId: `${prefix}owner`,buttonText:{displayText:'OWNER💪🗿'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: tamnel, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
}
             reply('Suksess broadcast')
}
             break
